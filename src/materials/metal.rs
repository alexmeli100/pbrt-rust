...
use crate::core::reflection::{BSDF, Fresnels, FresnelConductor, BxDFs, MicrofacetReflection};
...
const COPPER_WAVE_LENGHTS: [Float; COPPER_SAMPLES] = [
...
const COPPERN: [Float; COPPER_SAMPLES] = [
...
const COPPERK: [Float; COPPER_SAMPLES] = [
...
lazy_static! {
    static ref COPPER_N: Spectrum = Spectrum::from_sampled(&COPPER_WAVE_LENGHTS, &COPPERN, COPPER_SAMPLES);
    static ref COPPER_K: Spectrum = Spectrum::from_sampled(&COPPER_WAVE_LENGHTS, &COPPERK, COPPER_SAMPLES);
}
...
pub struct MetalMaterial {
    ...
    uroughness      : Option<Arc<TextureFloat>>,
    vroughness      : Option<Arc<TextureFloat>>,
...
impl MetalMaterial {
    pub fn new(
        eta: Arc<TextureSpec>, k: Arc<TextureSpec>, roughness: Arc<TextureFloat>,
        uroughness: Option<Arc<TextureFloat>>, vroughness: Option<Arc<TextureFloat>>,
        bump_map: Option<Arc<TextureFloat>>, remap_roughness: bool) -> Self {
        Self {
            eta, k, roughness, uroughness,
            vroughness, bump_map , remap_roughness
        }
    }
}
...
impl Material for MetalMaterial {
    fn compute_scattering_functions<'b: 'b>(
        ...
        let mut vrough = if let Some(ref v) = self.vroughness {
            v.evaluate(si)
        } else {
            self.roughness.evaluate(si)
        };
        ...
        let etat = self.eta.evaluate(si);
        let k = self.k.evaluate(si);
        let fres: &mut Fresnels = arena.alloc(FresnelConductor::new(&Spectrum::new(1.0), &etat, &k).into());

        let distrib: &mut MicrofacetDistributions = arena.alloc(TrowbridgeReitzDistribution::new(urough, vrough, true).into());
        let bxdf: &mut BxDFs = arena.alloc(MicrofacetReflection::new(&Spectrum::new(1.0), distrib, fres).into());
        bsdf.add(bxdf);
        si.bsdf = Some(bsdf)
    }
}

pub fn create_metal_material(mp: &mut TextureParams) -> Materials {
    let eta = mp.get_spectrumtexture("eta", *COPPER_N);
    let k = mp.get_spectrumtexture("k", *COPPER_K);
    let roughness = mp.get_floattexture("roughness", 0.01);
    let uroughness = mp.get_floattexture_ornull("uroughness");
    let vroughness = mp.get_floattexture_ornull("vroughness");
    let bump_map = mp.get_floattexture_ornull("bumpmap");
    let remap_roughness = mp.find_bool("remaproughness", true);

    MetalMaterial::new(eta, k, roughness, uroughness, vroughness, bump_map, remap_roughness).into()
}