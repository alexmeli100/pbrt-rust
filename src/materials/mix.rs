...
use std::sync::Arc;
use crate::core::texture::{Textures, Texture, TextureSpec};
...
impl Material for MixMaterial {
    fn compute_scattering_functions<'a: 'a>(
        ...
        self.m1.compute_scattering_functions(si, arena, mode, allow_multiple_lobes);
        self.m2.compute_scattering_functions(&mut si2, arena, mode, allow_multiple_lobes);

        let b1 = si.bsdf.as_mut().unwrap();
        let b2 = si2.bsdf.as_mut().unwrap();
        let n1 = b1.num_components(BxDFType::All);
        let n2 = b2.num_components(BxDFType::All);
