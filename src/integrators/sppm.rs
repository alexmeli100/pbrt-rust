                    ...
                    .for_each_init(|| herd.get(), |arena, Point2i { x, y }| {
                        ...
                        for ppixel in &tbounds {
                            ...
                            for depth in 0..self.max_depth {
                                ...
                                Ld += beta * uniform_sample_onelight(&i, scene, &arena, &mut tsampler, false, None);
                                ...
                                if is_diffuse || (is_glossy && depth == self.max_depth - 1) {
                                    vis_bsdf = Some(bsdf);
                                    vispoint = VisiblePoint::new(isect.p, wo, pixel_offset as usize, beta);
                                    break;
                                }
                            ...
                            pixs.push((pixel_offset, Ld, vispoint, vis_bsdf));
                        }
...
impl VisiblePoint {
    pub fn new(p: Point3f, wo: Vector3f, bsdf: usize, beta: Spectrum) -> Self {
        Self { p, wo, bsdf, beta }
    }
}
...
impl<'a> SPPMPixelListNode<'a> {
    pub fn new(pixel: &'a SPPMPixel) -> Self {
        Self {
            pixel,
